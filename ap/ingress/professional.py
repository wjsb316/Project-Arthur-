"""ProfessionalBrain policy and permission flow endpoints."""
...
logger = logging.getLogger("arthur.ap.professional")
...
def build_professional_router(permission_repository: PermissionRepository) -> APIRouter:
    ...
    """
    router = APIRouter(prefix="/professional/v1")
    ...
    async def request_side_effect(message: Dict[str, Any] = Body(...)) -> Dict[str, Any]:
        ...
        await permission_repository.create_request(
    ...
    async def permission_response(message: Dict[str, Any] = Body(...)) -> Dict[str, Any]:
        ...
        record = await permission_repository.get_request(request_id)
        if record is None:
            logger.warning(
                "ap_event",
        ...
        if now >= record.expires_at:
            await permission_repository.set_status(request_id, "expired")
            raise HTTPException(
        ...
        if decision == "no":
            await permission_repository.set_status(request_id, "denied")
            logger.info(
                "ap_event",
                extra={"event": "permission_denied", "severity": "info", "request_id": request_id},
            )
        else:
            await permission_repository.set_status(request_id, "approved")
            await permission_repository.record_delivery(request_id, record.client_label, record.note)
            logger.info(
                "ap_event",
                extra={
                    "event": "note_delivery",
...
__all__ = ["build_professional_router"]
